...
Dans le cadre de notre formation, Mr. Payet nous a demandé de réaliser le graphique le plus moche possible. Pour se faire, nous avons utilisé les données de starwars directement sur R grace au package dplyr. La base données a `r nrow(starwars)` lignes. De plus nous devions utiliser ggplot2 pour construire le graphique. en effet il est plus simple de modifier un graphique fait à partir de ggplot que avec plot.

## visualisation starwars
```{r data}
data("starwars")
```
